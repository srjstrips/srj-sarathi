...
import { Users, Plus, Search, Filter, MoreHorizontal, X, Mail, Phone, Building2, ChevronDown, Download, Loader2, AlertCircle } from 'lucide-react';
...
function normalizeEmployee(raw: any): Employee {
  const designation = raw.designation?.title ?? raw.designationName ?? raw.designation ?? '';
  const department  = raw.department?.name   ?? raw.departmentName  ?? raw.department  ?? '';
  const manager     = raw.manager?.firstName ? `${raw.manager.firstName} ${raw.manager.lastName ?? ''}`.trim() : undefined;
  const joinDate    = raw.joiningDate ?? raw.joinDate ?? raw.hireDate;
  return {
    id:          raw.id,
    code:        raw.employeeCode ?? raw.code ?? '',
    ...
    joinDate:    joinDate ? new Date(joinDate).toLocaleDateString('en-IN') : undefined,
  };
}
      ...
      <div className="relative bg-white w-full max-w-md shadow-2xl flex flex-col">
        <div className="p-5 border-b border-[#E2E0DC] bg-gradient-to-r from-orange-50 to-white">
          ...
          <div className="mt-3 flex items-center gap-2">
            ...
            {emp.department && <span className="text-xs text-[#757575]">{emp.department}</span>}
          </div>
        </div>
          ...
          {tab === 'overview' ? (
            <div className="space-y-4">
              <div className="space-y-3">
                {emp.email && (
                  <a href={`mailto:${emp.email}`} className="flex items-center gap-3 p-3 rounded-lg border border-[#E2E0DC] hover:border-orange-200 transition-colors">
                ...
                {emp.phone && (
                  <a href={`tel:${emp.phone}`} className="flex items-center gap-3 p-3 rounded-lg border border-[#E2E0DC] hover:border-orange-200 transition-colors">
              ...
              <div className="grid grid-cols-2 gap-3">
                {[['Manager', emp.manager], ['Join Date', emp.joinDate], ['Designation', emp.designation], ['Employee Code', emp.code]].map(([l, v]) => v ? (
        ...
        <div className="p-4 border-t border-[#E2E0DC] flex gap-2">
          <Link href={`/employees/${emp.id}`}
            className="flex-1 py-2 text-center bg-orange-500 hover:bg-orange-600 text-white text-sm font-medium rounded-lg transition-colors">
            Full Profile
          </Link>
        </div>
      </div>
    ...
    <div className="space-y-4">
      <div className="flex items-center justify-between">
        <div>
          ...
          <p className="text-sm text-[#757575] mt-0.5">{loading ? '...' : `${total} total employees`}</p>
        </div>
      ...
      <div className="grid grid-cols-3 gap-3">
        {[
          { label: 'Active',   count: employees.filter(e=>e.status==='ACTIVE').length,   cls: 'bg-green-50 text-green-700' },
          { label: 'On Leave', count: employees.filter(e=>e.status==='ON_LEAVE').length, cls: 'bg-orange-50 text-orange-700' },
          { label: 'Inactive', count: employees.filter(e=>e.status==='INACTIVE').length, cls: 'bg-gray-100 text-gray-600' },
        ].map(s => (
      ...
      <div className="flex flex-wrap items-center gap-3">
        ...
        <div className="relative">
          <select value={deptFilter} onChange={e => setDeptFilter(e.target.value)}
            className="appearance-none pl-3 pr-8 py-2 border border-[#E2E0DC] rounded-lg text-sm bg-white focus:outline-none focus:ring-2 focus:ring-orange-500/30 cursor-pointer">
      ...
      {loading ? (
        <div className="bg-white rounded-xl border border-[#E2E0DC] py-16 text-center">
          ...
          <p className="text-sm text-[#757575]">Loading employees...</p>
        </div>
      ) : error ? (
        <div className="bg-white rounded-xl border border-[#E2E0DC] py-16 text-center">
          <AlertCircle size={32} className="text-red-400 mx-auto mb-3" />
          <p className="text-[#1A1A1A] font-medium">Failed to load employees</p>
                ...
                <tr className="border-b border-[#E2E0DC] bg-[#F8F7F4]">
                  <th className="p-3 w-10">
                    <input type="checkbox" className="rounded"
                      onChange={e => setSelectedRows(e.target.checked ? new Set(filtered.map(f=>f.id)) : new Set())} />
                  </th>
                ...
                <div key={emp.id} onClick={() => setSelected(emp)} className="bg-white rounded-xl border border-[#E2E0DC] p-4 cursor-pointer hover:border-orange-200 transition-all">
                  <div className="flex items-start gap-3">
                    ...
                    <div className="flex-1 min-w-0">
                      ...
                      {emp.manager && <p className="text-xs text-[#ABABAB] mt-1">Manager: {emp.manager}</p>}
                    </div>
                  </div>
                </div>
              );
